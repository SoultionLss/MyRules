...
- **策略名称**: REJECT
- **规则总数**: 782 条
- **规则来源**: blackmatrix7/ios_rule_script@master, ACL4SSR/ACL4SSR@master

## 文件说明
- `REJECT.list`  → Surge / Loon / Egern 通用（后缀匹配，每行 .domain）
- `REJECT.yaml`  → Clash RULE-SET 格式（payload: 列表）
- `REJECT_domain.txt` → v2ray 纯域名列表（每行一个域名）
...
Surge / Loon / Egern:
在 [Rule] 部分添加：
RULE-SET, https://cdn.jsdelivr.net/gh/SoultionLss/MyRules@Rules/dist/REJECT/REJECT.list, REJECT

Clash:
在 rules 部分添加：
- RULE-SET, https://cdn.jsdelivr.net/gh/SoultionLss/MyRules@Rules/dist/REJECT/REJECT.yaml, REJECT
